...
func parseSQLMigration(r io.Reader, direction bool) (stmts []string, useTx bool, unsafe bool, err error) {
	...
	for scanner.Scan() {
		...
		if strings.HasPrefix(line, "--") {
			...
			case "+goose Up":
				...
				default:
					return nil, false, false, errors.Errorf("duplicate '-- +goose Up' annotations; stateMachine=%v, see https://github.com/random1st/goose#sql-migrations", stateMachine)
				}
				continue
			...
			case "+goose Down":
				...
				default:
					return nil, false, false, errors.Errorf("must start with '-- +goose Up' annotation, stateMachine=%v, see https://github.com/random1st/goose#sql-migrations", stateMachine)
				}
				continue
			...
			case "+goose StatementBegin":
				...
				default:
					return nil, false, false, errors.Errorf("'-- +goose StatementBegin' must be defined after '-- +goose Up' or '-- +goose Down' annotation, stateMachine=%v, see https://github.com/random1st/goose#sql-migrations", stateMachine)
				}
				continue
			...
			case "+goose StatementEnd":
				...
				default:
					return nil, false, false, errors.New("'-- +goose StatementEnd' must be defined after '-- +goose StatementBegin', see https://github.com/random1st/goose#sql-migrations")
				}
		...
		if _, err := buf.WriteString(line + "\n"); err != nil {
			return nil, false, false, errors.Wrap(err, "failed to write to buf")
		}
		...
		default:
			return nil, false, false, errors.Errorf("failed to parse migration: unexpected state %q on line %q, see https://github.com/random1st/goose#sql-migrations", stateMachine, line)
		}
	...
	if err := scanner.Err(); err != nil {
		return nil, false, false, errors.Wrap(err, "failed to scan migration")
	}
	// EOF

	switch stateMachine.Get() {
	case start:
		return nil, false, false, errors.New("failed to parse migration: must start with '-- +goose Up' annotation, see https://github.com/random1st/goose#sql-migrations")
	case gooseStatementBeginUp, gooseStatementBeginDown:
		return nil, false, false, errors.New("failed to parse migration: missing '-- +goose StatementEnd' annotation")
	}

	if bufferRemaining := strings.TrimSpace(buf.String()); len(bufferRemaining) > 0 {
		return nil, false, false, errors.Errorf("failed to parse migration: state %q, direction: %v: unexpected unfinished SQL query: %q: missing semicolon?", stateMachine, direction, bufferRemaining)
	}

	return stmts, useTx, unsafe, nil
}
